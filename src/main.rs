...
fn run(matches: ArgMatches) -> Result<(), String> {
    ...
    let dst: &str = matches
        ...
        .expect("Could not read dst address");
    let ip = format!("{}:{}", dst, port);

    let millis: u64 = matches
        .value_of("delay")
        .unwrap()
        .parse()
        .expect("Could not read delay time");

    let hop_addr: std::net::SocketAddr = hop_port
        .parse()
        .expect("Unable to parse SocketAddr from hop");
    let addr: &str = &ip;
    let mut ok = false;
